...
def init_three_layer_convnet(input_shape=(3, 32, 32), num_classes=10,
  ...
  filter_size = 5
  model = {}
  model['W1'] = np.random.randn(F1, 3, filter_size, filter_size)# * sqrt(2.0/(3*filter_size**2))
  model['b1'] = np.random.randn(F1)
  model['W2'] = np.random.randn(H * W * F1 / 4, FC)# * sqrt(2.0/(H*W*F1/4))
  model['b2'] = np.random.randn(FC)
  model['W3'] = np.random.randn(FC, num_classes)# * sqrt(2.0/FC)
