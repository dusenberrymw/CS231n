...
class LinearClassifier:
  ...
  def train(self, X, y, learning_rate=1e-3, reg=1e-5, num_iters=100,
    ...
    for it in xrange(num_iters):
      ...
      #########################################################################
      # choose batch_size indices from np.arange(num_train)
  ...
  def predict(self, X):
    ...
    Inputs:
    - X: D x N array of example data. Each column is a D-dimensional point.
    ...
    ###########################################################################
    scores = self.W.dot(X) # C x N array of class scores for each example
    y_pred = np.argmax(scores, axis=0) # select the class with highest score
                                       # for each example
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
